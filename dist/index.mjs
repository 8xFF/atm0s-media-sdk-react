...
import Atm0s, { StreamRemote } from '@8xff/atm0s-media-js';
...
var logger = getLogger('DeviceHook');
...
var useDevices = function (kind) {
    var _a = useState([]), devices = _a[0], setDevices = _a[1];
...
var SessionProvider = function (props) {
    ...
    var connect = useCallback(function (url, config) {
        var _a, _b;
    ...
    var getPublisher = useCallback(function (ownerId, cfg) {
        var _a;
    ...
    var backPublisher = useCallback(function (ownerId, cfg) {
        var data = storage.data;
        if (data === null || data === void 0 ? void 0 : data.session) {
            var publisher = data.publishers.get(cfg.name);
            if (publisher) {
    ...
    var getConsumer = useCallback(function (ownerId, stream) {
        var _a;
        var data = storage.data;
        if (data === null || data === void 0 ? void 0 : data.session) {
            var key = stream.peerId + '-' + stream.name;
    ...
    var backConsumer = useCallback(function (ownerId, stream) {
        var data = storage.data;
        if (data === null || data === void 0 ? void 0 : data.session) {
            var key = stream.peerId + '-' + stream.name;
            var consumer = data.consumers.get(key);
    ...
    var getConsumerPair = useCallback(function (ownerId, peerId, audioName, videoName) {
        var _a;
        var data = storage.data;
        if (data === null || data === void 0 ? void 0 : data.session) {
    ...
    var backConsumerPair = useCallback(function (ownerId, peerId, audioName, videoName) {
        var data = storage.data;
        if (data === null || data === void 0 ? void 0 : data.session) {
            var key = peerId + '-' + audioName + '-' + videoName;
            var consumer = data.consumerPairs.get(key);
...
var useRemoteStreams = function (kind, isMine) {
    var data = useContext(SessionContext).data;
...
var useSessionState = function () {
    var data = useContext(SessionContext).data;
    return (data === null || data === void 0 ? void 0 : data.state) || SessionState.New;
};

var useActions = function () {
    var _a = useContext(SessionContext), connect = _a.connect, disconnect = _a.disconnect;
    return {
        connect: connect,
        disconnect: disconnect,
...
var useConsumer = function (remote, priority, maxSpatial, maxTemporal) {
    var consumerId = useMemo(function () { return idSeed$1++; }, []);
    var sessionState = useSessionState();
    var _a = useState(), consumer = _a[0], setConsumer = _a[1];
    var _b = useState(Atm0s.StreamReceiverState.NoSource), state = _b[0], setState = _b[1];
    var _c = useState(), element = _c[0], setElement = _c[1];
    var _d = useContext(SessionContext), data = _d.data, getConsumer = _d.getConsumer, backConsumer = _d.backConsumer;
    var isConnectionEstablished = [SessionState.Connected, SessionState.Reconnecting].indexOf(sessionState) >= 0;
    useEffect(function () {
...
var useConsumerPair = function (peerId, audioName, videoName, priority, maxSpatial, maxTemporal) {
    ...
    var _b = useState(Atm0s.StreamReceiverState.NoSource), state = _b[0], setState = _b[1];
    var _c = useState(), element = _c[0], setElement = _c[1];
    var _d = useContext(SessionContext), data = _d.data, getConsumerPair = _d.getConsumerPair, backConsumerPair = _d.backConsumerPair;
    var isConnectionEstablished = [SessionState.Connected, SessionState.Reconnecting].indexOf(sessionState) >= 0;
    useEffect(function () {
        if (data === null || data === void 0 ? void 0 : data.session) {
...
var useLocalConsumer = function (stream) {
    var _a = useState(), element = _a[0], setElement = _a[1];
    useEffect(function () {
        if (element && stream) {
...
var usePublisher = function (cfg) {
    var publisherId = useMemo(function () { return idSeed++; }, []);
    var _a = useState(Atm0s.StreamSenderState.Created), state = _a[0], setState = _a[1];
    var _b = useState(), producer = _b[0], setProducer = _b[1];
    var _c = useContext(SessionContext), data = _c.data, getPublisher = _c.getPublisher, backPublisher = _c.backPublisher;
    useEffect(function () {
        if (data === null || data === void 0 ? void 0 : data.session) {
...
var useAudioLevelConsumer = function (consumer) {
    var _a = useState(undefined), audioLevel = _a[0], setAudioLevel = _a[1];
    useEffect(function () {
        if (consumer) {
...
var useAudioSlotMix = function (slotIndex) {
    var _a = useState(undefined), slot = _a[0], setSlot = _a[1];
    var data = useContext(SessionContext).data;
    useEffect(function () {
...
export { SessionContext as BlueseaSessionContext, LocalViewer, RemoteViewer, SessionProvider, SessionState, StreamPublisherWrap, VideoViewer, getDevice, useActions, useAudioLevelConsumer, useAudioLevelMix, useAudioLevelProducer, useAudioSlotMix, useConsumer, useConsumerPair, useDevice, useDevices, useDisplayMedia, useLocalConsumer, usePublisher, useRemoteStreams, useSession, useSessionState, useUserMedia };
